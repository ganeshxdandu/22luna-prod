...
export interface ConcernCTAProps {
  concern: ConcernDetailData;
  className?: string;
}

export function ConcernCTA({ concern, className }: ConcernCTAProps) {
  const title = concern.ctaTitle ?? `Discuss ${concern.name} In Consultation`;
  const subtitle =
    concern.ctaSubtitle ??
